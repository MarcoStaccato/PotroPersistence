...
public class PotrosMain {
	...
	public static void main(String[] args) {
		...
		userDAO.persist(us2);
		userDAO.persist(us3);
		System.out.println("***persisted users*** " + us1.getNombre() + "*"
				+ us2.getNombre() + "*" + us3.getNombre() + "*");
		...
		Compra comp;
		comp = new Compra(1, new Date(System.currentTimeMillis()), c,
				c.getUsuario());
		...
		for (int i = 0; i < departamentos.size(); i++) {
			System.out.print(((Departamento) departamentos.get(i))
					.getDescripcion() + " ");
		}
